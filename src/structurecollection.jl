...
A `StructureCollection` can be constructed from an underlying `BindingRules`.
"""
...
"""
    StructureCollection(sys::BindingRules; veff=1e-2, enum_kwargs...)
...
"""
function StructureCollection(sys::BindingRules; veff=1e-2, enum_kwargs...)
    strs = polygen(sys; enum_kwargs...)
    M = reduce(vcat, composition.(strs)')
    Zs = orientational_volume(dimension(sys)) * inv.(symmetrynumber.(strs)) / veff
